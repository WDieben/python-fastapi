...
from api.deps import DbDependency, user_dependency
...
@router.get("/")
def get_routines(db: DbDependency, user: user_dependency):
    return db.query(Routine).options(joinedload(Routine.workouts)).filter(Routine.user_id == user.get('id')).all()

@router.post("/")
def create_routine(db: DbDependency, user: user_dependency, routine: RoutineCreate):
    ...
    db_routines = db.query(Routine).options(joinedload(Routine.workout)).filter(Routine.id == db_routine.id).first()
    return db_routines

@router.delete("/")
def delete_routine(db: DbDependency, user: user_dependency, routine_id: int):
